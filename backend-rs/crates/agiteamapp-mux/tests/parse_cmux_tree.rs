//! Phase 0: core/discovery.rs 에서 이관된 cmux tree 텍스트 파서 검증.
//! 파서(parse_cmux_tree)는 **구조만** 추출한다(역할/terminal 필터는 core 책임).
//! 따라서 여기서는 비터미널 surface 도 is_terminal=false 로 포함됨을 확인한다.

use agiteamapp_mux::parse_cmux_tree;

#[test]
fn parse_cmux_tree_extracts_structure() {
    let tree = concat!(
...
#[test]
fn parse_cmux_tree_empty_input() {
    assert!(parse_cmux_tree("").is_empty());
}

#[test]
fn parse_cmux_tree_surface_before_workspace_ignored() {
    // workspace 라인 이전의 surface 는 소속 없음 → 무시(현행 cur_proj 가드 동등).
    let tree = "    ├── surface surface:9 [terminal] \"고아(PM)\"\n";
    assert!(parse_cmux_tree(tree).is_empty());
}

...
//  - pulse 수신 → runtimeActivity='active', activityBlinkKey+1, 3000ms 타이머 리셋.
//  - 3000ms 동안 새 pulse 가 없으면 클라가 스스로 idle 로 자연 정지한다(서버발 idle 미수신 전제).
//  - 서버발 idle 이벤트는 무시한다(되살아나도 무시 — §9 마지막 규칙).
//  - WS 재연결 gap replay 로 받은 과거 pulse(occurred_at 이 now 기준 3000ms+ 오래됨)는 재시작하지 않는다.
...
// 있었다(유저 실측 2026-06-15). 2000ms 로 늘려 완화했다(CSS 0.4s × 약 5회 = 2초, 새 pulse 시 0 리셋).
// 유저 요청(2026-06-16)으로 2000ms→3000ms 상향: pulse 간격이 더 벌어지는 구간에서도 끊김 없이
// 'LIVE·동작중' 점이 더 오래 유지되도록 체감 지속시간을 늘린다(CSS 0.4s × 약 7~8회 = 3초).
export const ACTIVITY_BLINK_MS = 3000;
...
//   설령 과거 pulse 가 와도 3초 후 자연 정지하므로 폭탄 방어는 그대로 성립한다.
...
//   3초 뒤 자연 정지하므로 오작동 없음. 단 degraded(mock)만 제외한다(가짜 깜빡 금지).
export function cardActivityState(room, ctx = {}) {
  if (!room || ctx.degraded) return null;
  // 반응성 heartbeat 는 store.nowTick(ctx.now) 의존으로 유지(컴포넌트가 매초 재평가 → 3초 자가정지).
  ...
  //   - lastActiveAt: REST degrade hint. now-3초 이내만 유지(§9).
